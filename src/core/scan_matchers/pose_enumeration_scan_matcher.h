...
public:
  ...
  double process_scan(const TransformedLaserScan &raw_scan,
    ...
    while (_pose_enumerator->has_next()) {
      RobotPose sampled_pose;
      if (_poses_are_iterative)
        sampled_pose = _pose_enumerator->next(best_pose);
      else
        sampled_pose = _pose_enumerator->next(init_pose);
    ...
    if(pose_delta)
      std::cout << "odom_fixed" << std::endl;
    return best_pose_prob;
  }

  void set_is_iterative(bool flag) {
    _poses_are_iterative = flag;
  }
private:
  std::shared_ptr<PoseEnumerator> _pose_enumerator;
  bool _poses_are_iterative = true;
