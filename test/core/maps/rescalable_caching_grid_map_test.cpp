...
#include "../../../src/slams/viny_mul/viny_grid_cell.h"
...
TEST_F(RescalableCachingGridMapTest, serializeDeserializeLittleMap) {
  auto map = TesteeMapType<>{cell_proto, {16, 16, 1}};
    map.set_scale_id(map.finest_scale_id());
  auto val = AreaOccupancyObservation{true, Occupancy{128, 0},
                                        Point2D{0, 0}, 1};
  auto updated_point = Point2D{15, 15};
  map.update(map.world_to_cell(updated_point), val);
  auto mega = map.save_map();

  map.load_map(mega);
  auto mega1 = map.save_map();

  ASSERT_EQ(mega, mega1);
  ASSERT_EQ(map.coarsest_scale_id(), 5);
  ASSERT_EQ(map.finest_scale_id(), 0);
}

TEST_F(RescalableCachingGridMapTest, serializeDeserializeBigMap) {
  auto map = TesteeMapType<>{cell_proto, {128, 128, 1}};
    map.set_scale_id(map.finest_scale_id());
  auto val = AreaOccupancyObservation{true, Occupancy{128, 0},
                                        Point2D{0, 0}, 1};
  auto updated_point = Point2D{100, 100};
  map.update(map.world_to_cell(updated_point), val);
  auto mega = map.save_map();
  ...
  ASSERT_EQ(mega, mega1);
  ASSERT_EQ(map.coarsest_scale_id(), 8);
  ASSERT_EQ(map.finest_scale_id(), 0);
}
